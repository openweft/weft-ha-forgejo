...
//
// The Forgejo HA agents need TWO things from the DCS :
//
//   - A CAS-style key store : the bootstrap leader writes the three
//     shared secrets (SECRET_KEY, INTERNAL_TOKEN, LFS_JWT_SECRET) under
//     `/weft/forgejo/<install>/secrets/...` exactly once ; followers
//     read them on boot and install verbatim.
//   - A lease-bound advisory lock around the bootstrap step itself :
//     concurrency.NewMutex on `/weft/forgejo/<install>/bootstrap-lock`
//     guarantees only one replica mints the secrets + migrates the
//     schema. The session's TTL means a fenced bootstrap leader
//     releases the lock automatically.
//
// This file is paid only when WEFT_HA_FORGEJO_ETCD is set — the
...
// installName is used as the key prefix so multiple Forgejo installs
// can share the same etcd cluster. sessionTTL is in seconds and
// defaults to 15 if non-positive — matches what weft-ha-postgresql
// uses for its leader-election lease.
func NewEtcdStore(endpoints []string, installName string, sessionTTL int) *EtcdStore {
	if sessionTTL <= 0 {
		sessionTTL = 15
	}
	return &EtcdStore{
...
func (e *EtcdStore) bootstrapLockPath() string {
	return path.Join("/weft/forgejo", e.installName, "bootstrap-lock")
}
...
// after secret minting completes).
//
